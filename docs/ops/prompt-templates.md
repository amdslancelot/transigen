...
> agent type 與 model 選法見 `docs/ops/model-dispatch.md` 派工對照表。
...
範圍：[src/ 全部 | 限定目錄]。只讀不改。
驗收條件：每個發現附 檔案:行號；如果找不到，列出你搜過的 pattern 與目錄（證明找過，不是漏找）。
...
回報格式：事實清單，每條 ≤ 2 行 + URL。長引文存 [scratchpad 路徑] 回傳路徑。
```

## 5. 審查（cavecrew-reviewer / fresh-context，sonnet）

```
背景：[這個 diff/檔案 是為了什麼]。
任務：審查 [diff 範圍 / 檔案清單]，只找 [正確性 bug | 弱模型會誤讀的模糊句 | 規則互相矛盾 | 過度工程]（選一個焦點，一次審一種）。
驗收條件：每個 finding 一行：位置 + 問題 + 建議修法 + 嚴重度。沒問題的部分不要寫（不要praise）。
回報格式：finding 清單按嚴重度排序；0 findings 就明說「檢查了 X/Y/Z 面向，無發現」。
特別檢查：[本次特有的風險點，例如「型號 ID 是否為查證值」]
```

## 派工後的主對話義務

- 收到回報先對驗收條件逐條核對，過了才採用；沒過按升降級路徑處理（`docs/ops/model-dispatch.md`）。
- 回報裡的結論要轉述給使用者時，用完整句子重述，不要原樣轉貼 subagent 的壓縮輸出。
